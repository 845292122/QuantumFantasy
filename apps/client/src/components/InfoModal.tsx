...
const InfoModal = <T extends InfoModalFieldType[]>({
  ...
  return (
    <>
      ...
      >
        <Form form={form} layout="vertical" initialValues={initialValues} preserve={false} autoComplete="off">
          <Row gutter={24}>
            {fields.map(field => (
              <Col span={field.span ?? 12} key={field.name}>
                <Form.Item name={field.name} label={field.label} rules={field.rules ?? []}>
                  {field.type === 'input' && <Input placeholder={`请输入${field.label}`} {...field.props} />}
                  {field.type === 'inputNumber' && <InputNumber {...field.props} />}
                  {field.type === 'select' && (
                    <Select placeholder={`请选择${field.label}`} allowClear options={field.options} {...field.props} />
                  )}
                  {field.type === 'radio' && (
